...
def determine_move_quality(abs_delta, delta, side_to_move, is_book, forced, is_sacrifice):
    """Helper function to determine the quality of a move"""
    if is_book:
        return "book"
    elif forced:
        return "forced"
    
    # Default quality classification based on Chess.com standards
    if abs_delta <= 20:
        quality = "best"  
    elif abs_delta <= 50:
        quality = "good"
    ...
    # Check for brilliant and great moves
    improvement = delta if side_to_move == chess.WHITE else -delta
    # Only consider brilliant and great moves when they involve a sacrifice
    if is_sacrifice and improvement > 300:
        quality = "brilliant"
    elif is_sacrifice and improvement > 100:
        quality = "great"
...
def analyze_game(pgn_data, stockfish_path=None, book_path=None):
    ...
    for i, move in enumerate(moves):
        ...
        # Check if this is one of the first 10 half-moves and if it's a book move
        is_book = False
        if i < 10:  # First 5 full moves (10 half-moves)
        ...
        # Get evaluation before move from cache
        ...
        # Get evaluation after move from cache
        ...
        quality = determine_move_quality(abs_delta, delta, side_to_move, is_book, forced, is_sacrifice)
        
        # Add this move to the analysis
        analysis.append({
            "ply": i + 1,
            "move": san,
            "quality": quality,
